...
Write-Host "`n[1/7] Stopping ECS service..." -ForegroundColor Yellow
...
Write-Host "`n[2/7] Deleting Athena workgroup..." -ForegroundColor Yellow
...
Write-Host "`n[3/7] Deleting ECR images..." -ForegroundColor Yellow
...
# Delete Aurora RDS stack first (depends on main stack VPC)
Write-Host "`n[4/7] Deleting Aurora RDS stack (~5 min)..." -ForegroundColor Yellow
aws cloudformation delete-stack --stack-name ai-rds-aurora --region $REGION 2>&1 | Out-Null
aws cloudformation wait stack-delete-complete --stack-name ai-rds-aurora --region $REGION 2>&1 | Out-Null
Write-Host "RDS stack deleted." -ForegroundColor Green

# Delete main CloudFormation stack
Write-Host "`n[5/7] Deleting main CloudFormation stack (~5 min)..." -ForegroundColor Yellow
...
Write-Host "`n[6/7] Deleting OIDC role stack..." -ForegroundColor Yellow
aws cloudformation delete-stack --stack-name github-actions-ai-agent-role --region $REGION 2>&1 | Out-Null
aws cloudformation wait stack-delete-complete --stack-name github-actions-ai-agent-role --region $REGION 2>&1 | Out-Null
Write-Host "OIDC stack deleted." -ForegroundColor Green

# Delete CloudWatch log group
Write-Host "`n[7/7] Deleting CloudWatch log group..." -ForegroundColor Yellow
...
$oidcStack = aws cloudformation describe-stacks --stack-name github-actions-ai-agent-role --region $REGION 2>&1
$rdsStack  = aws cloudformation describe-stacks --stack-name ai-rds-aurora --region $REGION 2>&1
...
if ($rdsStack -match "does not exist") { Write-Host "  RDS stack   : deleted" -ForegroundColor Green }
else { Write-Host "  RDS stack   : still exists!" -ForegroundColor Red }
